...
const emit = defineEmits<{
  itemClick: [index: number, item: LineSidebarItem];
}>();
...
function handleClick(index: number, item: LineSidebarItem): void {
  activeIndex.value = index;
  emit("itemClick", index, item);
  startLoop();
}

watch(activeIndex, startLoop, { flush: "post" });
      ...
      >
        ...
        >
          <span v-if="showIndex" class="line-sidebar__index">
            {{
              item.numbering.slice(1).join(".") ||
              String(index + 1).padStart(2, "0")
            }}
          </span>
          <span class="line-sidebar__text whitespace-nowrap text-sm">
            {{ item.value }}
          </span>
        </a>
      </li>
...
.line-sidebar__label {
  ...
  display: flex;
  min-width: 0;
  max-width: calc(100% - var(--max-shift));
